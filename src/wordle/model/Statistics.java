...
public class Statistics {
	...
	public Statistics(WordleModel model) {   // take model as argument in order to get the current mode
		String mode = model.getCurrentMode();   // get current mode, so can know dealing with which .log
		this.wordsGuessed = new ArrayList<>();
		String fileSeparator = System.getProperty("file.separator");
		this.path = System.getProperty("user.home") + fileSeparator + "Wordle";
		this.log = fileSeparator + mode + "_statistics.log";   // write/read stats from different .log, corresponding to current mode
		readStatistics();
	}
