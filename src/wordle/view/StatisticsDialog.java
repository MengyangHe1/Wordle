...
public class StatisticsDialog extends JDialog {
	...
	private JPanel createTopPanel() {
		...
		// if user fails game, show answer, otherwise don't show it
		if(model.getStatistics().getCurrentStreak()==0) {
			panel.add(createAnswerPanel(), BorderLayout.CENTER);
		}
	...
	private JPanel createTitlePanel() {
		...
		String mode = model.getCurrentMode();
		// Create different labels for different modes
		JLabel label = new JLabel("Statistics of " + mode.toUpperCase() + " Mode");
	...
	// Panel to show the answer when user fails the game
	private JPanel createAnswerPanel() {
		JPanel panel = new JPanel(new FlowLayout());
		panel.setBorder(BorderFactory.createEmptyBorder(0, 5, 5, 5));
		String answer = this.model.getCurWord();
		JLabel label = new JLabel("You FAILED! Answer is: " + answer);
		label.setFont(AppFonts.getTitleFont());
		panel.add(label);
		return panel;
	}
